...
void UpdateFontTexture()
{
    ...
    io.Fonts->TexID = (void*)(std::size_t)texture.getNativeHandle();
}
...
void Image(const sf::Texture& texture, const sf::Vector2f& size,
    const sf::Color& tintColor, const sf::Color& borderColor)
{
    ImGui::Image((void*)(std::size_t)texture.getNativeHandle(), size, ImVec2(0, 0), ImVec2(1, 1), tintColor, borderColor);
}
...
// @todo jonny fix all of this. Does it even need to use GL? can't it be done with sfml vertices?
                     ...
                     const sf::Color& bgColor, const sf::Color& tintColor)
{
    ...
    return ImGui::ImageButton((void*)(std::size_t)texture.getNativeHandle(), size, uv0, uv1, framePadding, bgColor, tintColor);
}
